<template>
    <div class="container full-screen d-flex align-items-center justify-content-center">
        <div class="card p-4 shadow-lg">
            ...
            <form @submit.prevent="requestPasswordReset" autocomplete="on">
                <div class="mb-3">
                    <label for="email" class="form-label">
                        Correo Electrónico
                    </label>
                    <input id="email" name="email" v-model="email" type="email" class="form-control" placeholder="Correo electrónico" required autocomplete="email">
                </div>
                <button class="btn btn-primary text-light mt-3 w-100">
                    Enviar enlace
                </button>
            </form>
            <p v-if="message" :class="'mt-3 text-' + status + ' text-center'">
                {{ message }}
            </p>
        </div>
    </div>
